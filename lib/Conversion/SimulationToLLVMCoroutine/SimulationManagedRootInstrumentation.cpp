...
/// Keeping one scalar clear per dead root is compact for ordinary functions,
/// but becomes quadratic in large coroutine bodies: UVM methods can have over
/// a thousand managed SSA roots and hundreds of collection points.  Beyond
/// this size, refresh the root array with one memset and bind only the values
/// live at the collection point.
...
LogicalResult instrumentManagedRoots(ModuleOp module) {
  ...
  if (failed(failableParallelForEach(
          context, functions, [&](sim::SimFuncOp function) -> LogicalResult {
            ...
            auto collectHandles = [&](Value value) -> LogicalResult {
              ...
              if (isa<sim::ManagedRefType>(value.getType()))
                slots.push_back(
                    {0, static_cast<uint32_t>(sim::ManagedHandleKind::Class),
                     false});
                    ...
                    {0,
                     static_cast<uint32_t>(sim::ManagedHandleKind::Class) |
                         static_cast<uint32_t>(
                             sim::ManagedHandleKind::ReferencePath),
                     false});
            ...
            bool useBulkRefresh = handles.size() > bulkRootRefreshThreshold;
            ...
            Value rootCount =
                llvmConstant(builder, location, i64, handles.size());
            ...
            // Each coroutine activation owns roots only while it is running on
            // the current worker lane. Pop before suspension and reacquire on
            // the resume block so a continuation may migrate to another host
            // thread.
            ...
            for (Block *block : activationBlocks) {
              ...
              Value runtimeContext = LLVM::LoadOp::create(
                  builder, location, pointer, contextAddress, 8);
              Value lane = LLVM::CallOp::create(
                               builder, location, TypeRange{pointer},
                               SymbolRefAttr::get(
                                   context, "obelisk_rt_v1_gc_current_lane"),
                               runtimeContext)
                               .getResult();
              ...
              LLVM::StoreOp::create(
                  builder, location,
                  LLVM::ZeroOp::create(builder, location, rootType), record, 8);
              Value status =
                  LLVM::CallOp::create(
                      ...
                      .getResult();
              Operation *check = reportManagedStatus(builder, location,
                                                     runtimeContext, status);
              check->setAttr(managedRootRangePushCheckAttr,
                             builder.getUnitAttr());
              last = check;
            ...
            for (auto [handle, slot] : llvm::zip_equal(handles, slots)) {
              // Exact managed words remain valid for the lifetime of their SSA
              // value, so binding once at the definition is sufficient. A
              // candidate word is arbitrary union storage: allocator reuse can
              // change whether it names a live object without changing the SSA
              // bits. Refresh that shadow at every collection point below
              // instead of caching classification here.
              if (useBulkRefresh || handle.slot.conditional)
                continue;
              if (auto argument = dyn_cast<BlockArgument>(handle.value)) {
                ...
                builder.setInsertionPointAfter(
                    cast<OpResult>(handle.value).getOwner());
              }
            ...
            // A root slot outlives its SSA value and is reused on loop
            // backedges. For ordinary functions, clear values that are dead at
            // each collection point and retain exact bindings made at
            // definitions. For large functions, clearing every dead slot
            // individually creates O(roots * safepoints) IR; bulk-clear the
            // range and rebuild only its live bindings instead.
            for (Operation *collectionPoint : collectionPoints) {
              ...
              if (useBulkRefresh)
                LLVM::MemsetOp::create(
                    builder, collectionPoint->getLoc(), slotsBase,
              ...
              for (auto [handle, slot] : llvm::zip_equal(handles, slots)) {
                ...
                if (isLive && (useBulkRefresh || handle.slot.conditional)) {
                  sim::SimClassRootBindOp::create(
