...
private:
  ...
  LogicalResult planTwoStateRegisters() {
    FailureOr<DenseSet<Value>> planned =
  ...
  LogicalResult encodeOperation(FunctionPlan &plan, Operation *operation) {
    ...
    if (auto op = dyn_cast<sim::SimStringFormatRealOp>(operation))
      return emitIntrinsic(plan, kIntrinsicStringFormatReal, {op.getInput()},
                           {op.getResult()});
    if (isa<sim::SimClassNullOp, sim::SimManagedNullOp,
            sim::SimCovergroupNullOp>(operation)) {
    ...
    if (auto op = dyn_cast<sim::SimCovergroupBinHitOp>(operation)) {
      ...
      return emitIntrinsicRegisters(
          plan, kIntrinsicCovergroupBinHit,
    ...
    if (auto op = dyn_cast<sim::SimCovergroupInstanceQueryOp>(operation))
      return emitIntrinsic(
          plan, kIntrinsicCovergroupInstanceQuery, {op.getHandle()},
  ...
  SmallVector<uint8_t> serializeBytecode() {
    return serializeBytecodeImage(
        plans, instructions, operandMaps, constants, intrinsicSignatures,
        intrinsicSites, captureRecords, state);
  }
