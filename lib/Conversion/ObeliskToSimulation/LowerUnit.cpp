...
private:
  ...
  findSemanticCovergroup(Operation *operation) const;
...
FailureOr<Value> UnitLowering::lowerNewCovergroup(
  ...
  FailureOr<Type> type = getNormalizedSemanticType(op);
  auto handleType =
      succeeded(type) ? dyn_cast<sim::CovergroupHandleType>(*type)
...
FailureOr<Value> UnitLowering::lowerCovergroupSample(
  ...
  for (semantic::SVCoverpointSymbolOp coverpoint : coverpoints) {
    ...
    if (members.size() < expressionCount) {
      emitError(pointLocation) << "coverpoint expression inventory is malformed";
      return failure();
    }
    ...
    FailureOr<Value> sampledScalar =
        succeeded(sampledValue)
            ? toPackedScalar(*sampledValue, getSemanticLocation(members.front()))
            : FailureOr<Value>(failure());
    ...
    if (logic) {
      ...
      Value bits = sim::SimLogicToBitsOp::create(builder, pointLocation,
                                                 bitsType, selector);
      Value roundTrip = sim::SimLogicFromBitsOp::create(
          builder, pointLocation, logicType, bits);
    ...
    auto compare = [&](Operation *candidateNode, sim::CompareKind logicKind,
      ...
      if (failed(candidate))
        return failure();
      FailureOr<Value> converted =
          convert(*candidate, (*sampledValue).getType(),
                  isSignedNode(candidateNode), candidateLocation,
                  isSignedNode(members.front()));
      FailureOr<Value> scalar =
          succeeded(converted)
              ? toPackedScalar(*converted, candidateLocation)
              : FailureOr<Value>(failure());
      ...
      if (logic) {
        Value result = sim::SimLogicCompareOp::create(
            ...
            sim::LogicType::get(function.getContext(), 1), logicKind,
            selector, *scalar);
        return sim::SimLogicIsTrueOp::create(
                   builder, candidateLocation, builder.getI1Type(), result)
            .getResult();
      }
    ...
    auto matchItem = [&](Operation *item) -> FailureOr<Value> {
      ...
      return compare(item, sim::CompareKind::Eq,
                     arith::CmpIPredicate::eq);
    };
    ...
    for (auto [binIndex, bin] : llvm::enumerate(bins)) {
      ...
      for (Operation *item : getChildren(bin)) {
        ...
        if (failed(matched))
          return failure();
        binMatched = arith::OrIOp::create(
            builder, getSemanticLocation(item), binMatched, *matched);
      }
      explicitMatched = arith::OrIOp::create(
          builder, getSemanticLocation(bin), explicitMatched, binMatched);
...
FailureOr<Value> UnitLowering::lowerCovergroupCall(
    semantic::SVCallExpressionOp op,
    semantic::SVCovergroupTypeOp covergroup) {
  Location location = getSemanticLocation(op);
  StringRef name = op.getCalleeName();
  ...
  bool instanceMethod = name == "sample" || name == "start" ||
                        name == "stop" || name == "get_inst_coverage";
  ...
  if (instanceMethod) {
    ...
    if (failed(handle) ||
        !isa<sim::CovergroupHandleType>((*handle).getType()))
      return failure();
  }
  ...
  bool noOutputs = llvm::all_of(*defaulted, [](int64_t value) {
    return value == 1;
  });
  bool twoOutputs = llvm::all_of(*defaulted, [](int64_t value) {
    return value == 0;
  });
  ...
  if (twoOutputs) {
    ...
    for (auto [actual, value] :
         llvm::zip_equal(outputs, queryOutputs)) {
...
UnitLowering::lowerSystemCall(semantic::SVCallExpressionOp op) {
  ...
  if (name == "$bitstoreal") {
    ...
    Value result = arith::BitcastOp::create(
        builder, location, builder.getF64Type(), *bits);
    return convertResult(result);
  }
  ...
  if (name == "$shortrealtobits") {
    ...
    if (failed(real))
      return failure();
    Value result = arith::BitcastOp::create(
        builder, location, builder.getI32Type(), *real);
    return convertResult(result);
  }
...
FailureOr<Value> UnitLowering::lowerExpression(Operation *op, bool lvalue) {
  ...
  if (isa<semantic::SVConversionExpressionOp>(op)) {
    ...
    if (isa<semantic::SVNullLiteralOp>(children.front())) {
      ...
      if (isa<sim::CovergroupHandleType>(*target))
        return sim::SimCovergroupNullOp::create(builder,
                                                getSemanticLocation(op),
                                                *target)
            .getResult();
    }
  ...
  if (auto construct =
          dyn_cast<semantic::SVNewCovergroupExpressionOp>(op))
