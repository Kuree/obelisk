...
FailureOr<ABIAlignments> validateTargetABI(ModuleOp module,
  ...
  if (failed(checkType("pointer", pointer, 8, 8)) ||
      ...
      failed(checkStruct("activation descriptor", {i64, pointer, i32, i32},
                         {0, 8, 16, 20}, 24, 8)) ||
      failed(checkStruct("observer capture ABI", {i32, i32}, {0, 4}, 8,
                         4)) ||
      ...
      failed(checkStruct("execution descriptor",
                         {i32, i32, i32, i32, pointer, i64, pointer, i64, i64,
                          i64, pointer, i64, i32, i32, pointer, i64, pointer,
                          i64},
...
public:
  ...
  matchAndRewrite(Operation *operation, ArrayRef<Value> operands,
    ...
    case RuntimeMaterializer::ArgumentManagedString: {
      ...
      FailureOr<Value> data = allocateAtFunctionEntry(
          operation, rewriter, abi, operands[0].getType(), 1,
          abi.alignments.i64);
      if (failed(data))
        return failure();
      LLVM::StoreOp::create(rewriter, location, operands[0], *data,
