...
assert.match(style, /\.pipeline\s+ol\s*{[^}]*width:\s*max-content;[^}]*min-width:\s*100%;/s);
assert.match(style, /\.pipeline\s+li\s*{[^}]*flex:\s*none;[^}]*padding:\s*0\s+13px;/s);
...
assert.match(app, /stackedMedia\.addEventListener\('change',\s*syncOrientation\)/);
assert.match(app, /if\s*\(stacked\(\)\)\s*ui\.workbench\.style\.gridTemplateColumns\s*=\s*''/);
assert.match(app, /ui\.handle\.setAttribute\('aria-orientation',\s*stacked\(\)\s*\?\s*'horizontal'\s*:\s*'vertical'\)/);
assert.match(app, /selected\?\.scrollIntoView\(\{\s*block:\s*'nearest',\s*inline:\s*'nearest'\s*\}\)/);
assert.match(app, /function\s+initPipelineScroll\(\)/);
assert.match(app, /pipeline\.classList\.toggle\(\s*'canDrag',\s*pipeline\.scrollWidth\s*>\s*pipeline\.clientWidth\s*\+\s*1/);
