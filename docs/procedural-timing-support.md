...
| Concurrent assertion cycle delays | Executable as compiler-generated bounded AOT state for `assert property`, `assume property`, `cover property`, `cover sequence`, and simulation-silent `restrict property`, with fixed `##N` delays and fixed consecutive `[*N]` repetition over a horizon of at most 63 clock samples. Ranged, unbounded, and dynamic delays are rejected rather than truncated or committed at an arbitrary endpoint. |
...
| Concurrent assertions | A bounded single-clock AOT-compiled slice is executable for `assert property`, `assume property`, `cover property`, `cover sequence`, and simulation-silent `restrict property`, with resolved default or explicit direct edge clocks, boolean sampled terms, fixed `##N`, fixed consecutive repetition, and atomic-antecedent overlapped/nonoverlapped implication. Predicates read the canonical Preponed snapshot and deterministic monitor state advances in Observed; explicit pass/fail actions and default `assert`/`assume` failures run as ordered detached Reactive callbacks. Cover implication reports only nonvacuous matches. Explicit and resolved default `disable iff` expressions over static design state are unsampled asynchronous observers: a true transition clears live bounded attempts, blocks starts while true, and invalidates queued same-slot callbacks; X/Z is not true. Statically spawned Observed monitors are native-AOT eligible, while forced AOT honestly retains the generic hybrid coordinator for cold Reactive reports and disable observers. `expect`, automatic disable operands, ranged/unbounded timing, multi-clock expressions, recursive or parameterized assertion instances, assertion locals, and dynamic timing remain targeted diagnostics. The monitor is ordinary verified simulation SSA with no runtime temporal interpreter or solver dependency. Labeled assertions preserve a stable path/ID for future assertion control without entering a procedural named-block activation. |
