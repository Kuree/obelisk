...
| Immediate assertions | Ordinary `assert`, `assume`, and `cover` lower to control flow with four-state truth semantics, explicit action blocks, and source-qualified default diagnostics for failed `assert` and `assume`. Deferred `#0`/`final` expressions are evaluated when encountered; named action inputs are captured by value. Reports use ticketed per-process/site replacement so only the last same-slot result matures. `#0` reports mature in Observed and explicit actions are spawned in Reactive; `final` reports and actions execute in read-only Postponed. Pending reports are flushed when their process resumes after a delay or event wait (including `always_comb`/`always_latch` reactivation) and when its outermost enclosing process scope is disabled; disabling a nested scope preserves them. Cancellation by disabling a specific assertion remains pending because resolved control-target identities are not yet mapped to deferred-report site IDs. Full subroutine-argument expression capture, assertion-control suppression, and immediate-cover evaluation/success counters also remain pending. |
| Concurrent assertions | A bounded single-clock AOT-compiled slice is executable only for simulation-silent `restrict property`, with resolved default or explicit direct edge clocks, boolean sampled terms, fixed `##N`, fixed consecutive repetition, and atomic-antecedent overlapped/nonoverlapped implication. Predicates read the canonical Preponed snapshot and deterministic monitor state advances in Observed. Observable `assert`, `assume`, `cover`, and `expect` directives are rejected before callback generation pending a verifier-backed ordered Observed-result to Reactive-report queue; the unsafe callback-fanout path is not exposed. `disable iff` is rejected because it requires asynchronous unsampled cancellation. Ranged/unbounded timing, multi-clock expressions, recursive or parameterized assertion instances, assertion locals, and dynamic timing remain targeted diagnostics. The monitor is ordinary verified simulation SSA with no runtime temporal interpreter or solver dependency. The forced native AOT scheduler still rejects these actors because its generic-ordering eligibility currently excludes non-Active processes; extending it to ordered Observed monitor actors is the next scheduler blocker. Labeled restrictions preserve a stable path/ID for future assertion control without entering a procedural named-block activation. |
