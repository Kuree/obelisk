...
| Concurrent assertion cycle delays | Executable only for simulation-silent `restrict property` as compiler-generated bounded AOT state for fixed `##N` delays and fixed consecutive `[*N]` repetition with a horizon of at most 63 clock samples. Ranged, unbounded, and dynamic delays are rejected rather than truncated or committed at an arbitrary endpoint. |
