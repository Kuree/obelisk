...
| Compiler Z3 planning | A pinned Z3 4.13.4 static library is built from source for the compiler only. It validates hard residual programs and diagnoses constraints that are unsatisfiable for every runtime capture value under a deterministic resource limit. The build uses single-threaded polling mode so compiler-side Z3 remains WebAssembly-friendly; Z3 is never linked into `libobelisk_rt`. With `OBELISK_ENABLE_Z3=OFF`, conservative analysis returns unknown and emits the identical runtime fallback. Domain narrowing and richer sampler-plan synthesis remain pending. |
