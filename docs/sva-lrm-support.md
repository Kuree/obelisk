...
| Repetition | Executable subset | Fixed and finite positive consecutive `[*N]` / `[*M:N]` repetition within the 63-sample horizon and 256-trace cap is compiled AOT. Common positive finite `[->M:N]` and `[=M:N]` forms over one boolean term are executable alone or with a deterministic bounded prefix and one `##1` boolean continuation. They use a fixed-size aggregate token DFA with at most O(N) integer state, not one runtime process per live attempt; action multiplicity is preserved by a counted Reactive dispatch loop. | Add `[*0]`, unbounded repetition, general continuations/composition, locals and match items, `first_match`/implication integration, cover-sequence per-endpoint accounting, and end-of-simulation completion for still-waiting attempts. |
