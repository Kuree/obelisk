...
| Property implication | Executable subset | `|->` and `|=>` work for deterministic bounded antecedent and consequent sequences within a combined 63-sample state horizon. Overlapping attempts use disjoint compiler-generated antecedent/consequent bitset state. Failed antecedent attempts produce vacuous assertion success but never a cover hit. | Add branching antecedent endpoint sets, vacuity control/accounting, followed-by forms, assertion-local flow through multi-cycle antecedents, and composition with all property operators. |
