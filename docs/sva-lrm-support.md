...
| Sampled-value functions | Executable subset | `$sampled`, `$past`, `$rose`, `$fell`, `$stable`, and `$changed` work on statically addressable packed storage. `$past` supports constant positive depth and a statically addressable gate, using bounded per-logical-process history. The compiler coalesces referenced canonical bit ranges so Preponed capture copies only sampled state. | Add explicit alternate clock arguments, automatic/computed operands, full clock-context validation, and the remaining global sampled-value functions. |
...
| `disable iff`, `accept_on`, `reject_on`, `sync_accept_on`, `sync_reject_on` | Semantic IR; rejected | Disable and abort expressions/actions are preserved. | Implement asynchronous unsampled cancellation for `disable iff`, sampled synchronous aborts, attempt teardown, and ordered result suppression. |
