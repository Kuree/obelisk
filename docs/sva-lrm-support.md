...
| Cycle delay `##` | Executable subset | Fixed nonnegative `##N` and small bounded `##[M:N]` ranges are compiled AOT with a total horizon of at most 63 samples. Branching expansion is capped at 256 exact traces. One final `##[M:$]` with `M <= 63`, a deterministic bounded prefix, and one Boolean terminal uses a bounded warm-up bitset plus one merged eligible-token count. Ordinary sequence properties take the first match, while `cover sequence` preserves every successful endpoint. Bare assert/assume sequences complete weakly and explicit `strong` completes pending tokens as failures at end of simulation. | Compose unbounded delay with implication, abort/disable, locals and branching operators; add larger endpoint sets without trace expansion and dynamic delay extensions if supported by policy. |
...
| Property implication and followed-by | Executable subset | `|->`, `|=>`, `#-#`, and `#=#` work for deterministic bounded antecedent and consequent sequences within a combined 63-sample state horizon. Overlapping attempts use disjoint compiler-generated antecedent/consequent bitset state. Failed implication antecedents produce vacuous assertion success but never a cover hit; failed followed-by antecedents report failure instead. | Add branching antecedent endpoint sets, vacuity control/accounting, assertion-local flow through multi-cycle antecedents, and composition with all property operators. |
