...
| `accept_on`, `reject_on`, `sync_accept_on`, `sync_reject_on` | Executable subset | `sync_accept_on` and `sync_reject_on` wrap a plain deterministic bounded property. The abort condition uses the clock's Preponed sample and takes priority over ordinary property evaluation. Every older live attempt and the attempt starting on that clock is completed exactly once, monitor state is cleared, accepted results schedule vacuous assertion success but not a cover hit, and rejected results schedule failure. | Implement asynchronous `accept_on` / `reject_on`, compose aborts with locals, branching and persistent properties, implication/followed-by, `disable iff`, `first_match`, `expect`, and cover-sequence per-match accounting, then integrate vacuous controls/counters. |
