...
2. Add a runtime representation for unbounded live attempts and extend the
   bounded final-phase closure to persistent/branching state. This unlocks
   unbounded delay/repetition and unbounded `eventually`/`always`, plus
   `strong`/`weak` composition beyond deterministic bounded traces.
