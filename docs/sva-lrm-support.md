...
| Other property operators | Executable subset | Deterministic one-cycle boolean operands support property `not`, `iff`, and `implies`. Conditional `if`/`else` properties support bounded branches; a missing else is retained as a vacuous success and does not create a cover-property hit. Equivalent same-endpoint boolean traces are minimized by compiler-side Z3 before monitor SSA is built; generated simulation has no solver dependency. IR preserves the remaining `case`, `until`, `s_until`, `until_with`, `s_until_with`, `nexttime`, `s_nexttime`, `always`, `s_always`, `eventually`, and `s_eventually` forms, which receive operator-specific unsupported diagnostics. | Generalize boolean property composition, add property `case`, then implement weak/strong termination, finite-simulation completion, vacuity, and the precise attempt/thread semantics of the remaining temporal operators. |
