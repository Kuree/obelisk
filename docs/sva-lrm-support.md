...
| Other property operators | Executable subset | Deterministic one-cycle boolean operands support property `not`, `iff`, and `implies`. Conditional `if`/`else` and ordinary case-equality `case` properties support bounded branches. Missing else/default branches are retained as vacuous successes and do not create cover-property hits; case items preserve source priority and four-state case equality. Fixed `nexttime` / `s_nexttime` and explicitly finite-range `always` / `s_always` / `eventually` / `s_eventually` compile to bounded traces; the strong/weak distinction when simulation ends before a trace completes remains part of the pending end-of-simulation closure. Equivalent same-endpoint boolean traces are minimized by compiler-side Z3 before monitor SSA is built; generated simulation has no solver dependency. IR preserves the remaining `until`, `s_until`, `until_with`, and `s_until_with` forms, which receive operator-specific unsupported diagnostics. | Generalize boolean property composition, then implement weak/strong termination, finite-simulation completion, vacuity, and the precise attempt/thread semantics of the remaining temporal operators. |
